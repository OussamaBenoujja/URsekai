...
-- ------------------------------------------
-- MONETIZATION AND TRANSACTION TABLES
-- ------------------------------------------
...
-- Table: user_items
-- Inventory of items owned by users
CREATE TABLE user_items (
    ...
    acquired_method ENUM('purchase', 'gift', 'reward', 'promotion', 'starter', 'other') DEFAULT 'purchase',
    ...
    trade_cooldown_until DATETIME,
    custom_properties JSON,
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    FOREIGN KEY (item_id) REFERENCES virtual_items(item_id) ON DELETE CASCADE,
...
CREATE TABLE user_subscriptions (
    ...
    status ENUM('active', 'canceled', 'expired', 'pending', 'failed') DEFAULT 'pending',
    start_date DATETIME NOT NULL,
    end_date DATETIME,
    auto_renew BOOLEAN DEFAULT TRUE,
    cancel_reason TEXT,
    canceled_at DATETIME,
    payment_method ENUM('credit_card', 'paypal', 'crypto', 'bank_transfer', 'other') NOT NULL,
    payment_details JSON,
...
-- Records of webhook delivery attempts
...
-- Table: review_votes
-- Tracks upvotes/downvotes on reviews
CREATE TABLE review_votes (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (review_id) REFERENCES game_reviews(review_id) ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    UNIQUE KEY (review_id, user_id)
) ENGINE=InnoDB;

-- Table: review_comments
-- Comments on game reviews
...
CREATE TABLE forum_posts (
    ...
    FOREIGN KEY (hidden_by) REFERENCES users(user_id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
-- Table: group_members
-- Maps users to groups they belong to
